...
useEffect(() => {
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-slate-800 rounded-3xl border border-slate-700 shadow-xl overflow-hidden transition-all duration-300">
              ...
              {showChildForm && (
                <div className="p-6 border-t border-slate-700/50 bg-slate-900/20 space-y-6 animate-fade-in">
                  <form
                    onSubmit={(e) => { e.preventDefault(); handleAddProfile(e); }}
            ...
            <div className="flex gap-2 w-full md:w-auto">
              ...
              <button
                ...
                className="flex-1 md:flex-none bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2"
              >
                <span>🖨️</span> Cetak Tracker
              </button>
                    
              {/* TOMBOL KELUAR LAMA */}
              <button
                onClick={handleLogout}
                className="flex-1 md:flex-none bg-slate-800 hover:bg-red-500/20 text-slate-300 hover:text-red-400 border border-slate-700 hover:border-red-500/50 px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-sm"
              >
              {t.btnExit}
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 p-6 md:p-12 font-sans pb-32 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full print:hidden">
        {currentRole === 'child' ? renderChildView() : renderParentView()}
      </div>
      <PrintableView profiles={profiles} tasks={tasks} rewards={rewards} />
      <div className="print:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-800/90 backdrop-blur-md p-1.5 rounded-full border border-slate-700 shadow-2xl flex items-center gap-1">
        <button
          type="button"
